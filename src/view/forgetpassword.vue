<template>
  <div class="page" role="main">
    <div class="nav-bar-center-wrap">
      <van-nav-bar title="重置密码" left-arrow @click-left="onClickLeft" class="nav-bar" />
    </div>
...
function sendSms() {
  ...
  API.sendSms({ mobile: phone.value, event: "forgetPass", captcha: imgCode.value, uniqid: uniqid.value })
    .then((res) => {
      if (res.code == 1) {
        startCountdown(60);
        showToast("验证码已发送");
