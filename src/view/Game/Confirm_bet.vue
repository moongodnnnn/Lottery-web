...
onMounted(() => {
  ...
  try {
    if (route.query.details) {
      ...
      const gameCount = Object.keys(groupedBets.value).length;
      if (gameCount === 1) {
        selectedBetType.value = "1x1"; // 单关
      } else if (gameCount === 2) {
        selectedBetType.value = "2x1"; // 2串1
      } else if (gameCount === 3) {
        selectedBetType.value = "3x1"; // 3串1
      } else if (gameCount === 4) {
        selectedBetType.value = "4x1"; // 4串1
      } else if (gameCount === 5) {
        selectedBetType.value = "5x1"; // 5串1
      } else if (gameCount === 6) {
        selectedBetType.value = "6x1"; // 6串1
      } else {
        selectedBetType.value = `${gameCount}x1`; // 默认全串1
      }
    } else {
      showToast("未找到投注数据");
      router.back();
