<template>
    <div class="page" role="main">
        <div class="nav-bar-center-wrap">
            <van-nav-bar title="排列5" left-arrow @click-left="onClickLeft" class="nav-bar">
                <template #right>
        ...
        <div class="betting-section">
            ...
            <div class="ball-section">
                <div class="section-header">
                    <span class="section-title">万位</span>
                    <span class="section-count">已选 {{ tenThousandBalls.length }} 个</span>
                </div>
                <div class="ball-grid-pl3">
                    <div v-for="num in 10" :key="'tenThousand-' + num"
                        :class="['ball', 'pl3-ball', { selected: tenThousandBalls.includes(num) }]"
                        @click="toggleTenThousandBall(num)">
                        {{ num }}
                    </div>
                </div>
            </div>
            ...
            <div class="ball-section">
                <div class="section-header">
                    <span class="section-title">千位</span>
                    <span class="section-count">已选 {{ thousandBalls.length }} 个</span>
                </div>
                <div class="ball-grid-pl3">
                    <div v-for="num in 10" :key="'thousand-' + num"
                        :class="['ball', 'pl3-ball', { selected: thousandBalls.includes(num) }]"
                        @click="toggleThousandBall(num)">
                        {{ num }}
                    </div>
                </div>
            </div>
            ...
            <div class="ball-section">
                ...
                <div class="ball-grid-pl3">
                    <div v-for="num in 10" :key="'hundred-' + num"
            ...
            <div class="ball-section">
                ...
                <div class="ball-grid-pl3">
                    <div v-for="num in 10" :key="'ten-' + num"
                        :class="['ball', 'pl3-ball', { selected: tenBalls.includes(num) }]" @click="toggleTenBall(num)">
                        {{ num }}
                    </div>
                </div>
            </div>
            ...
            <div class="ball-section">
                ...
                <div class="ball-grid-pl3">
                    <div v-for="num in 10" :key="'unit-' + num"
            ...
            <div class="action-buttons">
                ...
                <div style="text-align: center">
                    ...
                    <div style="font-size: 0.7rem; color: #999">
                        <span v-if="!canConfirm">万位、千位、百位、十位、个位至少各选1个</span>
                        <span v-else>万{{ tenThousandBalls.length }} 千{{ thousandBalls.length }} 百{{ hundredBalls.length
                            }} 十{{
                            tenBalls.length }} 个{{ unitBalls.length }}</span>
                    </div>
                </div>
        ...
        <van-popup v-model:show="showRule" position="bottom" :style="{ height: '60%' }">
            <div class="rule-popup">
                ...
                <div class="rule-content">
                    <div class="rule-item">
                        <h3>玩法介绍</h3>
                        <p>排列5是从万位、千位、百位、十位、个位五个位置分别选择0-9的数字进行投注的玩法。</p>
                    </div>
                    <div class="rule-item">
                        <h3>投注规则</h3>
                        <p>• 万位：从0-9中至少选择1个号码</p>
                        <p>• 千位：从0-9中至少选择1个号码</p>
                        <p>• 百位：从0-9中至少选择1个号码</p>
                        <p>• 十位：从0-9中至少选择1个号码</p>
                        <p>• 个位：从0-9中至少选择1个号码</p>
                        <p>• 单注金额：2元</p>
                        <p>• 注数计算：万位数量 × 千位数量 × 百位数量 × 十位数量 × 个位数量</p>
                    </div>
                    <div class="rule-item">
                        <h3>开奖时间</h3>
                        <p>每天晚上20:30开奖</p>
                    ...
                    <div class="rule-item">
                        <h3>示例</h3>
                        <p>选择万位：1，千位：2，百位：3，十位：4，个位：5、6</p>
                        <p>共1×1×1×1×2=2注，金额4元</p>
                        <p>组合：12345、12346</p>
                    </div>
                </div>
            </div>
...
const tenThousandBalls = ref([]); // 万位
const thousandBalls = ref([]); // 千位
...
const canConfirm = computed(() => {
    return tenThousandBalls.value.length >= 1 &&
        thousandBalls.value.length >= 1 &&
        hundredBalls.value.length >= 1 &&
        tenBalls.value.length >= 1 &&
        unitBalls.value.length >= 1;
});

// 计算注数
const betCount = computed(() => {
    const tenThousandCount = tenThousandBalls.value.length;
    const thousandCount = thousandBalls.value.length;
    const hundredCount = hundredBalls.value.length;
    const tenCount = tenBalls.value.length;
    const unitCount = unitBalls.value.length;

    if (tenThousandCount < 1 || thousandCount < 1 || hundredCount < 1 || tenCount < 1 || unitCount < 1) {
        return 0;
    }

    // 排列5注数 = 万位数量 × 千位数量 × 百位数量 × 十位数量 × 个位数量
    return tenThousandCount * thousandCount * hundredCount * tenCount * unitCount;
});
...
// 万位选择逻辑
function toggleTenThousandBall(num) {
    const index = tenThousandBalls.value.indexOf(num);
    if (index > -1) {
        tenThousandBalls.value.splice(index, 1);
    } else {
        tenThousandBalls.value.push(num);
        tenThousandBalls.value.sort((a, b) => a - b);
    }
}

// 千位选择逻辑
function toggleThousandBall(num) {
    const index = thousandBalls.value.indexOf(num);
    if (index > -1) {
        thousandBalls.value.splice(index, 1);
    } else {
        thousandBalls.value.push(num);
        thousandBalls.value.sort((a, b) => a - b);
    }
}
...
function confirmSelection() {
    if (!canConfirm.value) {
        showToast("万位、千位、百位、十位、个位至少各选1个");
        return;
    }

    // 判断是单式还是复式
    // 单式：万位、千位、百位、十位、个位各选1个
    // 复式：任意位置选择超过1个
    const isSingle = tenThousandBalls.value.length === 1 &&
                     thousandBalls.value.length === 1 &&
                     hundredBalls.value.length === 1 &&
                     tenBalls.value.length === 1 &&
                     unitBalls.value.length === 1;
    ...
    // 生成所有组合
    const combinations = [];
    for (const tt of tenThousandBalls.value) {
        for (const th of thousandBalls.value) {
            for (const h of hundredBalls.value) {
                for (const t of tenBalls.value) {
                    for (const u of unitBalls.value) {
                        combinations.push(`${tt}${th}${h}${t}${u}`);
                    }
                }
            }
        }
    ...
    const lotteryData = {
        ...
        betData: {
            zx: [{
                1: tenThousandBalls.value,  // 万位
                2: thousandBalls.value,      // 千位
                3: hundredBalls.value,       // 百位
                4: tenBalls.value,           // 十位
                5: unitBalls.value           // 个位
            }]
        },
        // 保留原有格式用于显示
        selections: {
            tenThousand: tenThousandBalls.value,
            thousand: thousandBalls.value,
...
function confirmRandomSelect(betCount) {
    ...
    if (betCount === 1) {
        // 随机1注：每个位置选1个号码
        tenThousandBalls.value = [Math.floor(Math.random() * 10)];
        thousandBalls.value = [Math.floor(Math.random() * 10)];
...
onMounted(() => {
    // 获取彩票数据
    API.lottery_pailie(1)
        .then((res) => {
            if (res.code === 1) {
                console.log("排列5数据:", res.data);
