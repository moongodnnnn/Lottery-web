<template>
  <div class="page" role="main">
    <div class="nav-bar-center-wrap">
      <van-nav-bar title="邀请好友" left-arrow @click-left="onClickLeft" class="nav-bar" />
    </div>
    <div style="height: 1px; background-color: #f5f5f5"></div>

    <div class="invite-top-img">
      <img src="/img/Invitebgtop.jpg" class="invite-top" alt="邀请顶部" />
    </div>
...
onMounted(async () => {
  try {
    ...
    if (userStr) {
      ...
      inviteLink.value = `${baseUrl}/register?recom=${code.value}`;
    }
  } catch (e) {
    showToast("获取邀请码失败");
  }
...
async function downloadImage() {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      topImg.src = "/img/Invitebgtop.jpg";
    });
    ...
    await new Promise((resolve, reject) => {
      ...
      bottomImg.src = "/img/Invitebgbottom.jpg";
    });
    ...
    const qrY = topHeight + bottomHeight * 0.05; // 距离底部图片顶部5%的位置
    ...
    canvas.toBlob((blob) => {
      ...
      a.href = url;
      a.download = `邀请好友-${code.value}.png`;
      a.click();
...
.copy-btn {
  flex: 1;
  height: 48px;
  background: linear-gradient(90deg, #ff272e, #d32121);
  color: #fff;
  border: none;
  border-radius: 24px;
  font-size: 16px;
